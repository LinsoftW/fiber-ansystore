...
import { useTranslation } from '../hooks/useTranslation';
import { useDevice } from '../context/DeviceContext';
// Eliminar esta importación - ya no se usa directamente
// import { ProjectService } from '../../service/storage';
import QRGeneratorModal from './QRGeneratorModal';
...
const ListaProyectos = ({ navigation }) => {
  ...
  const loadProyectos = async () => {
    try {
      setLoading(true);
      console.log('🔄 Cargando proyectos desde AsyncStorage...');
      const projects = await getProjects();
  ...
  const deleteProyecto = async (proyecto) => {
    console.log('🗑️ deleteProyecto called with proyecto:', proyecto);
            ...
            try {
              ...
              for (const proyecto of proyectos) {
                try {
                  ...
                  console.error('❌ Error deleting project:', proyecto.id, error);
                  errorCount++;
                }
              }
    ...
    return (
      ...
      >
        <View style={styles.proyectoItemHeader}>
          ...
          <TouchableOpacity
            onPress={(e) => {
              console.log('🗑️ Delete button pressed for project:', item.id);
              e?.stopPropagation?.();
              deleteProyecto(item);
            }}
            style={styles.actionButton}
    ...
    <View style={[stylesFull.screen, { backgroundColor: colors.background }, { paddingBottom: bottomInset }]}>
      ...
      {filteredProyectos.length === 0 ? (
        <View style={styles.emptyState}>
          ...
          <Text style={[styles.emptyStateSubtext, { color: colors.subText }]}>
            {t('createProjectsToSeeHere') || 'Crea un proyecto para verlo aquí'}
          </Text>
        </View>
      ) : (
        <FlatList
