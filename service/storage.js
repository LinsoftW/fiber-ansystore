...
export const FiberService = {
  // Obtener todas las fibras
  getAllFibers: async () => {
    try {
      const fibers = await StorageService.getItem('@fibraoptica/fibers');
      return fibers || [];
    } catch (error) {
      console.error('❌ Error getting all fibers:', error);
      return [];
    }
  },

  // Obtener fibras por proyecto y parentId
  getFibersByProject: async (projectId, parentId = null) => {
    try {
      const allFibers = await FiberService.getAllFibers();
      return allFibers.filter(fiber => {
        const matchesProject = fiber.projectId === projectId;
        const matchesParent = parentId === null ? !fiber.parentId : fiber.parentId === parentId;
        const notDeleted = !fiber.deleted;
        return matchesProject && matchesParent && notDeleted;
      });
    } catch (error) {
  ...
  createFiber: async (fiberData) => {
    try {
      ...
      allFibers.push(newFiber);
      await StorageService.setItem('@fibraoptica/fibers', allFibers);
  ...
  updateFiber: async (fiberId, updates) => {
    try {
      const allFibers = await FiberService.getAllFibers();
      const fiberIndex = allFibers.findIndex(f => f.id === fiberId);
      
      if (fiberIndex === -1) {
        throw new Error('Fiber not found');
      }

      allFibers[fiberIndex] = {
        ...allFibers[fiberIndex],
        ...updates,
        modifiedDate: new Date().toISOString()
      };
  ...
  // Obtener fibra por ID
  getFiberById: async (fiberId) => {
    try {
      const allFibers = await FiberService.getAllFibers();
      return allFibers.find(fiber => fiber.id === fiberId && !fiber.deleted) || null;
    } catch (error) {
  ...
  // Eliminar todas las fibras de un proyecto
  deleteFibersByProject: async (projectId) => {
    try {
      const allFibers = await FiberService.getAllFibers();
      const updatedFibers = allFibers.filter(fiber => fiber.projectId !== projectId);
      
      await StorageService.setItem('@fibraoptica/fibers', updatedFibers);
      return { success: true };
    } catch (error) {
      console.error('❌ Error deleting project fibers:', error);
      throw error;
    }
  }
};
...
export const MediaService = {
  ...
  getAllMedia: async () => {
    try {
      const media = await StorageService.getItem('@fibraoptica/media');
      return media || [];
    } catch (error) {
  ...
  // Obtener medios por nodeId
  getMediasByNodeId: async (nodeId) => {
    try {
      const allMedia = await MediaService.getAllMedia();
      return allMedia.filter(media => media.nodeId === nodeId && !media.deleted);
    } catch (error) {
  ...
  createMedia: async (mediaData) => {
    try {
      ...
      await StorageService.setItem('@fibraoptica/media', allMedia);
  ...
  getMediaById: async (mediaId) => {
    try {
      const allMedia = await MediaService.getAllMedia();
      return allMedia.find(media => media.id === mediaId && !media.deleted) || null;
    } catch (error) {
...
export const initializeStorage = async () => {
  try {
    ...
    for (const key of keys) {
     ...
     // Inicializar las nuevas keys para fibers y media
    const fibersData = await StorageService.getItem('@fibraoptica/fibers');
    if (fibersData === null) {
      await StorageService.setItem('@fibraoptica/fibers', []);
    }

    const mediaData = await StorageService.getItem('@fibraoptica/media');
    if (mediaData === null) {
      await StorageService.setItem('@fibraoptica/media', []);
    }
